...
The Critic operates in a loop: \texttt{collect\_feedback()} inspects the DAG for structural gaps, \texttt{critique()} sends the DAG and gap report to an LLM backend, and \texttt{apply\_revisions()} validates and applies proposed changes. Three backends are supported: Claude Code CLI (default, no API key required), Codex CLI, and direct API via \texttt{LLMClient}.

Each revision has one of 8 types from a closed set: \texttt{ADD\_MISSING\_NODE}, \texttt{ADD\_MISSING\_EDGE}, \texttt{REMOVE\_EDGE}, \texttt{CHANGE\_EDGE\_TYPE}, \texttt{CHANGE\_EDGE\_SIGN}, \texttt{UPDATE\_NODE\_METADATA}, \texttt{UPDATE\_EDGE\_METADATA}, and \texttt{ANNOTATE}. All revisions must pass \texttt{CriticRevisionValidator} before application, which enforces a confidence threshold (default 0.7) and structural consistency checks. Revisions below the confidence threshold are logged but not applied.

\subsection{Quality Score}

A 6-component weighted score tracks DAG quality across iterations:
...
\midrule
Literature grounding & 20\% & Fraction of edges with $\geq 1$ citation \\
Completeness & 20\% & Fraction of edge card slots populated \\
