...
%% ============================================================
\begin{abstract}
We introduce \oc{}, an open-source platform that resolves the automation--transparency tradeoff in applied causal inference. \oc{} combines DAG-based causal reasoning with agentic estimation pipelines, where every modeling decision---from identification strategy selection to effect propagation---is logged in a hash-chained audit trail. An always-on \emph{sentinel loop} runs continuously in the background, re-validating the DAG after every change, auto-fixing schema regressions, and surfacing interactive review panels---so analysts never have to hunt for results or wonder whether the specification is still valid. The platform enforces 38 issue-detection rules that catch overclaiming, control shopping, and specification drift; a 7-guardrail propagation engine that performs unit-aware dimensional analysis across multi-edge causal paths; and a 3-stage identifiability screen that downgrades claims based on diagnostic evidence rather than statistical significance alone. A 4-agent agentic loop (DataScout, ModelSmith, Estimator, Judge) iterates between estimation, issue detection, and auto-repair under LLM guidance, while an LLM-assisted pipeline extracts causal claims from the literature and proposes DAG edges---achieving 100\% estimate precision on matched edges in our Kazakhstan bank stress-testing case study while discovering 9 novel edges absent from the expert-built DAG. A principle-based \emph{DAG Critic} loop iteratively refines DAG structure using LLM-generated feedback against domain-agnostic graph rules and four chain-level \emph{causal logic probes} (sign coherence, scale consistency, magnitude plausibility, sign-vs-estimate), supported by a \emph{KnowledgeScout} agent that auto-researches missing identity formulas via LLM and a \emph{DAGHealthSnapshot} eval framework that tracks per-edge health before and after each auto-fix iteration to detect regressions. A data-availability oracle prevents the critic from proposing edges where no data loaders exist. A Model Context Protocol (MCP) query server exposes the full propagation engine to external AI assistants. All governance decisions---including human-in-the-loop gates and automated patch policies that explicitly prohibit p-hacking---are recorded with cryptographic integrity. \oc{} ships with 19 estimation adapters spanning econometric and ML-based methods, a data-driven causal discovery agent (PC, GES, FCI, NOTEARS), an adaptive data discovery agent that uses LLM tool-use to find data sources for under-specified DAG nodes, a DoWhy refutation engine for post-estimation robustness, a natural-language query interface, and 341 passing tests across 82{,}000 lines of code.
\end{abstract}
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
    \item A \textbf{DAG-based causal inference platform} with 19 estimation adapters (econometric and ML-based), mode-aware effect propagation, unit-dimensional analysis across 8 named unit types, and 7 guardrails that gate edge usage based on identification strength, time-series diagnostics, and issue severity (Section~\ref{sec:propagation}).

    \item A \textbf{38-rule issue detection engine} with a PatchPolicy whitelist that explicitly prohibits control shopping, sample trimming, lag searching, and outcome switching---preventing automated p-hacking while permitting safe auto-fixes (Section~\ref{sec:issues}).
    ...
    \item A \textbf{principle-based DAG Critic} that iteratively refines DAG structure via LLM-generated revisions governed by domain-agnostic graph rules, four chain-level causal logic probes (sign coherence, scale consistency, magnitude plausibility, sign-vs-estimate), a 7-component quality score, and confidence-gated validation---operating at a higher governance tier than PatchBot. A \textbf{KnowledgeScout} agent auto-researches missing identity formulas via LLM, and a \textbf{DAGHealthSnapshot} eval framework tracks per-edge health to detect regressions (Section~\ref{sec:critic}).

    \item A \textbf{Model Context Protocol (MCP) query server} that exposes 13 tools---propagation, falsification, edge inspection, and health checks---to external AI assistants via stdio transport, with full guardrail enforcement and a read-only contract (Section~\ref{sec:mcp}).
\end{enumerate}

We evaluate \oc{} on two case studies: a Kazakhstan bank stress-testing DAG (32 nodes, 21 edges) with full estimation, and a Venezuelan oil supply unlock DAG (11 nodes, 13 edges) demonstrating cross-domain applicability with validated literature estimates. We compare expert-built DAGs against both LLM-extracted and data-driven discovery DAGs, and verify pipeline reproducibility across 341 tests (Section~\ref{sec:experiments}).
...
\begin{figure}[t]
    ...
    \node[component] at (2.8, 1.5) {38-Rule Issues};
...
Table~\ref{tab:adapters} lists all 19 adapters. The registry supports both built-in mappings and YAML-configurable dispatch (\texttt{design\_registry.yaml}), enabling new estimators to be added without modifying core code. All ML-based adapters use lazy imports---\texttt{dowhy}, \texttt{doubleml}, \texttt{econml}, and \texttt{causalml} are only loaded when the corresponding design is dispatched.

\begin{table}[t]
\centering
\caption{Estimation adapters in \oc{}. The registry supports 19 adapters spanning econometric and ML-based methods. All share a unified \texttt{EstimationRequest}~$\to$~\texttt{EstimationResult} interface.}
\label{tab:adapters}
\scriptsize
\begin{tabular}{@{}llll@{}}
\toprule
...
\oc{} assigns each edge a \emph{claim level} from a 4-tier hierarchy: \texttt{IDENTIFIED\_CAUSAL} $>$ \texttt{REDUCED\_FORM} $>$ \texttt{DESCRIPTIVE} $>$ \texttt{BLOCKED\_ID}. The central design principle is that \textbf{claim levels are a one-way ratchet: they can only be maintained or downgraded, never upgraded, after estimation begins.} This reflects the foundational insight that identification comes from the research design, not from the data~\citep{angrist2010credibility}. Good standard errors do not make an OLS estimate causal; a large $t$-statistic does not substitute for an instrument.

Concretely, the ratchet works as follows. First, the estimation \emph{design} sets a ceiling: IV and RCT designs permit \texttt{IDENTIFIED\_CAUSAL}; local projections and panel FE permit at most \texttt{REDUCED\_FORM}; OLS permits only \texttt{DESCRIPTIVE}. Second, post-estimation diagnostics can only \emph{cap downward} via a monotone \texttt{cap\_to()} operator---a failed leads test forces \texttt{BLOCKED\_ID}, a weak first-stage $F$ forces \texttt{REDUCED\_FORM}, but no diagnostic success ever triggers an upgrade. Third, human-in-the-loop review (Section~\ref{sec:governance}) can confirm or further downgrade the claim, but the HITL panel offers no mechanism to promote a claim level beyond its design-determined ceiling. Upgrading a claim requires changing the research design itself---introducing an instrument, adding an exclusion restriction---which means re-specifying the DAG and re-running estimation from scratch: a deliberate, auditable act recorded in the hash-chained ledger, not a post-hoc checkbox.
...
\oc{} ships with 38 issue-detection rules loaded from a YAML registry. Each rule specifies a severity (\texttt{CRITICAL}, \texttt{HIGH}, \texttt{MEDIUM}, \texttt{LOW}), a scope (edge, node, DAG, run, or cross-run), and whether the issue is auto-fixable or requires human review.
...
\midrule
\texttt{SIG\_NOT\_ID} & HIGH & edge & $p<0.05$ but claim $\neq$ \texttt{IDENTIFIED\_CAUSAL} \\
...
%% 9  DAG CRITIC LOOP
%% ============================================================
\section{Principle-Based DAG Critic}
\label{sec:critic}

Sections~\ref{sec:llm}--\ref{sec:discovery} construct an initial DAG from text or data. The \textbf{DAG Critic} iteratively refines this structure by generating LLM-based feedback against domain-agnostic graph principles, applying validated revisions, and converging when quality stabilizes.

\subsection{Architecture}

The Critic operates in a two-phase loop. First, a \textbf{deterministic auto-fix pass} (\texttt{apply\_auto\_fixes()}) runs identity claim upgrades and unit chain mismatch repairs in a converging inner loop (max 5 iterations) until no further auto-fixes are produced. These corrections require no LLM reasoning: definitional edges (identity, mechanical, bridge, immutable) are forced to \texttt{IDENTIFIED\_CAUSAL}, and their unit specifications are adapted to match neighboring edges' units. Second, \texttt{collect\_feedback()} inspects the DAG for structural gaps and propagation health, \texttt{critique()} sends the DAG, gap report, and propagation diagnostics to an LLM backend, and \texttt{apply\_revisions()} validates and applies proposed changes. Three backends are supported: Claude Code CLI (default, no API key required), Codex CLI, and direct API via \texttt{LLMClient}.

Each revision has one of 10 types from a closed set: \texttt{UPGRADE\_EDGE\_TYPE}, \texttt{ADD\_IDENTIFICATION\_STRATEGY}, \texttt{FIX\_FORMULA}, \texttt{ADD\_MISSING\_NODE}, \texttt{ADD\_MISSING\_EDGE}, \texttt{ADD\_STRUCTURAL\_METADATA}, \texttt{ADD\_UNIT\_SPECIFICATION}, \texttt{INVERT\_EDGE\_DIRECTION}, \texttt{CREATE\_EDGE\_CARD}, and \texttt{UPDATE\_CLAIM\_LEVEL}. All revisions must pass \texttt{CriticRevisionValidator} before application, which enforces a confidence threshold (default 0.7) and structural consistency checks. Revisions below the confidence threshold are logged but not applied.

\subsection{Quality Score}

A 7-component weighted score tracks DAG quality across iterations:

\begin{table}[t]
\centering
\caption{DAG Critic quality score components.}
\label{tab:critic-quality}
\small
\begin{tabular}{@{}llp{5.5cm}@{}}
\toprule
\textbf{Component} & \textbf{Weight} & \textbf{Measurement} \\
\midrule
Identification coverage & 20\% & Fraction with claim $\geq$ \texttt{REDUCED\_FORM} \\
Propagation health & 20\% & Fraction of open paths from exogenous shocks to target in \texttt{REDUCED\_FORM} mode \\
Metadata completeness & 15\% & Fraction of edge card slots populated \\
Structural soundness & 15\% & DAGValidator pass rate $\times$ gap penalty \\
Edge type diversity & 10\% & Entropy of edge type distribution \\
Unit completeness & 10\% & Fraction with treatment + outcome units \\
Formula correctness & 10\% & Fraction of identity edges with valid formulas \\
\bottomrule
\end{tabular}
\end{table}

The loop converges when the quality delta falls below 0.02 for 2 consecutive iterations, when zero revisions are proposed, or when the maximum iteration count is reached.

\subsection{Structural Gap Detection}

Four domain-agnostic graph rules detect structural incompleteness:

\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item \textbf{Undriven identity dependency}: An identity edge's input has no incoming causal driver, leaving part of the decomposition unexplained.
    \item \textbf{Dead-end leaf}: A non-target leaf node with no outgoing edges---either missing downstream effects or a modeling error.
    \item \textbf{Endogenous root}: A root node marked as endogenous, which should have incoming edges explaining its variation.
    \item \textbf{Paper-mentioned variable}: A variable referenced in the literature but absent from the DAG, suggesting a structural gap.
\end{enumerate}

Each detected gap contributes a 5\% penalty to the structural soundness score (capped at 50\%), providing quantitative pressure to resolve gaps across iterations.

\subsection{Causal Logic Probes}
\label{sec:probes}

While per-edge diagnostics (Section~\ref{sec:tsguard}) catch estimation-level issues, they cannot detect \emph{chain-level} inconsistencies---contradictions that only emerge when examining how edges interact across the DAG. The Critic addresses this with four domain-agnostic \textbf{causal logic probes}, computed from edge card estimates and DAG structure during each \texttt{collect\_feedback()} call:

\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item \textbf{Sign coherence}: For each node with two or more incoming edges carrying \texttt{expected\_sign} metadata, flags cases where signs conflict (one positive, one negative to the same target). Contradictory convergent paths indicate either a modeling error or a genuinely ambiguous economic mechanism requiring documentation.

    \item \textbf{Scale consistency}: Parses unit specifications via \texttt{UnitSpec.parse()} and flags (a)~non-unity scales (informational: the edge operates in a non-standard unit) and (b)~unit-kind mismatches between consecutive edges in a chain (warning: the outcome unit of edge $i$ does not match the treatment unit of edge $i+1$).

    \item \textbf{Magnitude plausibility}: From edge card point estimates, flags $|\hat{\beta}| > 10$ (possibly mis-scaled---e.g., level data estimated where log was intended) or $|\hat{\beta}| < 0.001$ with $p < 0.05$ (negligible but statistically significant, warranting economic significance review).

    \item \textbf{Sign vs.\ estimate}: Compares the DAG-declared \texttt{expected\_sign} against the estimated coefficient sign from the edge card. Contradictions are flagged as warnings; however, edges with high $p$-values or null-at-horizon-zero IRF trajectories that show correct signs at later horizons are typically not bugs but rather reflect estimation noise at impact.
\end{enumerate}

Probe findings are \emph{informational only}: they do not alter the quality score, introduce new revision types, or block propagation. Instead, they are formatted into the LLM critic's user prompt, providing concrete code-computed evidence that the LLM cannot derive on its own. The LLM may then propose revisions using existing types (\texttt{ADD\_UNIT\_SPECIFICATION}, \texttt{ADD\_STRUCTURAL\_METADATA}, etc.) to address probe findings.

\subsection{Governance Tier}

The Critic operates at a \emph{higher governance tier} than PatchBot (Section~\ref{sec:patchpolicy}). Specifically, the Critic can change edge types---an action explicitly forbidden for PatchBot. The Sentinel (Section~\ref{sec:sentinel}) never expands DAG structure; only the Critic proposes \texttt{ADD\_MISSING\_NODE} and \texttt{ADD\_MISSING\_EDGE} revisions. This separation ensures that automated schema repair (PatchBot) and structural expansion (Critic) remain distinct governance acts with different audit signatures.

\subsection{Propagation-Aware Feedback}
\label{sec:critic-propagation}

A DAG with perfect metadata but no open causal paths is functionally useless. The Critic addresses this by computing \textbf{propagation health}---the fraction of directed paths from exogenous shock nodes to the target node that are open in \texttt{REDUCED\_FORM} mode---as part of each \texttt{collect\_feedback()} call. Blocked paths are decomposed into specific guardrail failures (missing edge cards, insufficient claim levels, unit mismatches, diagnostic-only roles) and surfaced to the LLM as actionable repair targets. Two dedicated revision types support this: \texttt{CREATE\_EDGE\_CARD} materializes placeholder cards for unestimated edges, and \texttt{UPDATE\_CLAIM\_LEVEL} upgrades edge claims when identification evidence supports it.

On the Kazakhstan narrative DAG, propagation-aware iteration unblocked all 9 paths from exchange rate shocks (\texttt{kzt\_usd}) to bank capital adequacy (\texttt{k2\_ratio\_kspi}) and both paths from global risk (\texttt{vix\_shock}) to the same target, across 4 iterations with 41 applied revisions. Three distinct FX transmission channels emerged: tradable CPI, nontradable CPI, and an imported inflation shift-share IV---each flowing through real income/expenditure to bank profitability and capital. After the full estimation-aware repair pipeline (Section~\ref{sec:critic-autofix}), all 33 directed paths from exogenous sources to the target are open in \texttt{REDUCED\_FORM} mode, with 91 ranked contributor paths.

\subsection{KnowledgeScout: LLM Formula Research}
\label{sec:knowledge-scout}

Identity edges require formulas to compute coefficients (partial derivatives), but NL-extracted DAGs often lack them. The \textbf{KnowledgeScout} agent addresses this by querying an LLM (Claude CLI by default) for domain knowledge when \texttt{\_compute\_identity\_coefficient()} encounters an identity edge with no formula.

The research flow: (1)~construct a structured prompt describing the target variable and its dependencies, (2)~parse the LLM response for a formula string and confidence score, (3)~reject low-confidence results ($<0.6$), (4)~write the formula to both the in-memory DAG and the YAML file on disk. KnowledgeScout slots into the coefficient hierarchy as step~3: K2 ratio $\to$ immutable validated evidence $\to$ \textbf{KnowledgeScout} $\to$ symbolic extraction $\to$ numerical perturbation. On the Kazakhstan DAG, KnowledgeScout correctly identified the CPI headline composition formula ($0.42 \times \text{cpi\_tradable} + 0.58 \times \text{cpi\_nontradable}$), enabling identity card creation for two previously LP-estimated edges.

\subsection{DAGHealthSnapshot Eval Framework}
\label{sec:critic-eval}

The auto-fix loop applies multiple corrections per iteration, but without before/after comparison, regressions go undetected. The \textbf{DAGHealthSnapshot} framework addresses this:

\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item \texttt{take\_snapshot()} captures per-edge health: credibility rating, point estimate, claim level, propagation role, and issue flags (SMALL\_N, WRONG\_SIGN, NEAR\_ZERO, WEAK\_ID, CF\_BLOCKED, DIAG\_FAIL). Also records the 7-component quality score and propagation health.
    \item \texttt{\_diff\_snapshots(before, after)} compares records: detects rating downgrades/upgrades, new/resolved issues, quality and propagation deltas. Sets \texttt{net\_regression = True} when quality drops by $>0.01$.
\end{enumerate}

Each auto-fix iteration logs: edges upgraded, edges downgraded, quality delta, and regression warnings. This is currently diagnostic-only (log, don't revert), providing visibility into the repair loop's behavior without blocking convergence.

\subsection{Estimation-Aware Auto-Repair}
\label{sec:critic-autofix}

Beyond the deterministic auto-fixes (identity claims, unit chains), the Critic runs an \emph{estimation-aware} repair pass that detects and fixes four categories of problematic edges:

\begin{enumerate}[leftmargin=*, itemsep=1pt]
    \item \textbf{Placeholder edges}: Identity/mechanical edges with LP-estimated cards (design mismatch). Fixed by computing identity coefficients via the formula hierarchy and writing IDENTITY cards.
    \item \textbf{Exploding magnitudes}: $|\hat{\beta}| > 100$, suggesting mis-scaled units.
    \item \textbf{Wrong signs}: Estimated sign contradicts the DAG-declared expected sign.
    \item \textbf{Dead channels}: $|\hat{\beta}| < 10^{-6}$, indicating no meaningful transmission.
\end{enumerate}

A data-availability oracle (\texttt{check\_edge\_data\_feasible()}) prevents the critic from proposing new edges for variables without data loaders, which was the root cause of silent estimation failures in earlier iterations.

On the Kazakhstan DAG, this pipeline raised quality from 0.668 to 0.894: CPI composition edges received correct identity coefficients ($\beta \approx 0.42$ and $0.58$, replacing LP estimates of $0.762$ and $0.722$), and 4 edges were re-estimated with corrected specifications.
...
To enable AI-assisted causal reasoning beyond the CLI, \oc{} includes an MCP (Model Context Protocol) query server that exposes the full propagation engine to external AI assistants such as Claude Code.

\subsection{Architecture}

The server runs as a long-lived stdio process exposing 13 tools organized into four categories: \emph{setup} (\texttt{load\_dag}), \emph{inspection} (\texttt{list\_nodes}, \texttt{list\_edges}, \texttt{inspect\_edge}, \texttt{get\_identification}), \emph{propagation} (\texttt{propagate\_shock}, \texttt{propagate\_policy}, \texttt{find\_paths}, \texttt{target\_contributors}), and \emph{diagnostics} (\texttt{switch\_mode}, \texttt{compare\_modes}, \texttt{run\_placebo}, \texttt{dag\_doctor}).

The server maintains a single-DAG state model: each call to \texttt{load\_dag} fully replaces all state---the DAG, edge cards, TSGuard results, issue ledger, and propagation engine. There is no cross-DAG state sharing. Artifact directories mirror the CLI layout (\texttt{outputs/agentic/\{dag\_stem\}/}), ensuring both interfaces read from the same estimation results.

\subsection{Guardrail Enforcement}

All propagation tools delegate to the same \texttt{PropagationEngine} used by the CLI, with all 7 guardrails (Section~\ref{sec:propagation}) applied. \texttt{propagate\_shock} passes \texttt{scenario\_type="shock"} (shock CF gating); \texttt{propagate\_policy} passes \texttt{scenario\_type="policy"} (requires STRUCTURAL mode and \texttt{IDENTIFIED\_CAUSAL} claim level). The MCP server never bypasses or relaxes guardrails---an AI assistant querying through MCP receives identically gated results as a human using the CLI.

\subsection{Read-Only Contract}

The server is strictly read-only: it never writes edge cards, issues, or DAG files. The only mutable state is the in-memory DAG (replaced on \texttt{load\_dag}) and the active query mode (changed by \texttt{switch\_mode}). This ensures that AI-assisted exploration cannot alter the governed estimation artifacts.

\subsection{Interpretation Guardrails}

While the propagation engine enforces guardrails on the \emph{computation} (which paths are blocked, which counterfactuals are eligible), a separate risk arises at the \emph{interpretation} stage---when the AI assistant translates JSON results into natural-language explanations. Common errors include using causal language for non-identified paths, omitting blocked paths, or confusing sign conventions.

\oc{} addresses this with a companion plugin (\texttt{econ-guardrails}) that intercepts MCP tool results via Claude Code's PostToolUse hook mechanism. When \texttt{propagate\_shock} or \texttt{propagate\_policy} returns, two hooks fire: (1)~a \emph{prompt hook} injects the hedged language rules from Section~23 of the project's invariants document---forbidden causal verbs, mode-appropriate framing, SE independence disclaimers, blocked-path reporting, and draft framing; (2)~a \emph{command hook} (\texttt{propagation\_check.py}) validates the raw JSON for structural issues: reaction-function edges on unblocked paths (an engine bug if present), all-blocked warnings, unit chain consistency, and magnitude plausibility.

This creates a four-layer defense: background skill (prevention), PostToolUse hooks (interpretation guard), Stop hooks (output audit), and an on-demand verification agent (deep review). The hooks operate entirely outside the MCP server---no changes to \texttt{query\_server.py} or \texttt{PropagationEngine}---preserving the server's domain-agnostic design.
...
\paragraph{Downgrade-only review.} A critical property of the HITL gate is that it enforces the one-way ratchet described in Section~\ref{sec:identification}. The available analyst actions---\emph{accept} (acknowledge with caveat), \emph{reject} (suppress edge), \emph{revise} (re-specify and re-estimate), \emph{escalate} (defer to senior reviewer)---all either maintain or reduce the edge's effective claim level. There is no ``approve as causal'' action. An analyst reviewing a \texttt{REDUCED\_FORM} edge cannot promote it to \texttt{IDENTIFIED\_CAUSAL}, regardless of how strong the point estimate appears. The ``revise'' action triggers re-estimation with a modified specification, and the \emph{new} design determines the new ceiling---the ratchet resets to the design's maximum, not to the analyst's preference. This ensures that post-hoc rationalization (running models first, then retrofitting a causal story) is structurally impossible within the governed workflow.
...
\begin{figure}[t]
    ...
    \caption{HITL Resolution Panel. The review board shows detected issues grouped by rule type and severity. Each issue card displays edge estimates, credibility rating, and action dropdown with justification field. Bulk actions and JSON export support the governance workflow.}
...
We evaluate \oc{} on four dimensions: (1)~estimation accuracy via synthetic benchmarks, (2)~NL-to-DAG extraction quality, (3)~cross-domain propagation with guardrail enforcement, and (4)~end-to-end pipeline reproducibility.
...
We compare an expert-built DAG (32~nodes, 21~edges, approximately 40~hours of construction time) against an LLM-extracted DAG from a single descriptive paragraph about Kazakhstan bank stress testing.
...
\begin{figure}[t]
    ...
    % Expert only
    \node[font=\footnotesize, text width=2cm, align=center] at (-2.5, 0) {18 edges\\{\scriptsize (oil shocks, policy\\rate, balance\\sheet, ...)}};

    % Overlap
    \node[font=\footnotesize, text width=1.8cm, align=center] at (0, 0) {\textbf{3 matched}\\{\scriptsize 100\% estimate\\precision}};

    % NL only
    \node[font=\footnotesize, text width=2.2cm, align=center] at (2.6, 0) {9 novel\\{\scriptsize VIX$\to$funding,\\income$\to$CoR,\\PPOP$\to$capital}};

    % Counts
    \node[font=\scriptsize, gray] at (-1.2, -2.2) {32 nodes, 21 edges};
    \node[font=\scriptsize, gray] at (1.2, -2.2) {18 nodes, 12 edges};
    \end{tikzpicture}
    \caption{Comparison of expert-built and NL-extracted DAGs for the Kazakhstan case study. The 3~matched edges achieve 100\% estimate precision. The NL pipeline discovers 9~novel edges, while missing 18~edges encoding upstream shock transmission and banking balance sheet mechanics.}
    \label{fig:nl2dag}
\end{figure}

The NL pipeline achieves 14\% structural recall---unsurprising given that a single paragraph cannot encode upstream shock transmission (oil supply/demand shocks to exchange rate), monetary policy mechanics, or banking balance sheet identities. However, all 3~matched edges produce statistically equivalent estimates, and the pipeline discovers 9~novel edges. Three are particularly notable:

\begin{itemize}[leftmargin=*, itemsep=1pt]
    \item Global risk appetite (VIX) $\to$ bank funding costs (risk-taking channel)
    \item Household real income $\to$ cost of risk (bank lending channel)
    \item Pre-provision operating profit $\to$ total capital (fundamental banking relationship)
\end{itemize}

\subsection{Venezuelan Oil Supply Unlock: Cross-Domain Demonstration}
\label{sec:venezuela}

To demonstrate domain agnosticism, we construct a second DAG modeling the causal transmission from Venezuelan sanctions relief to global oil prices (Figure~\ref{fig:venezuela-dag}). The Venezuela DAG contains 11~nodes and 13~edges across 6~channels: direct production restart, FDI-investment pipeline, OPEC strategic response (reaction function), non-OPEC supply growth, supply-demand price formation, and a heavy-sour crude discount channel.
...
\begin{figure}[t]
    ...
    \begin{tikzpicture}[
        ...
        nbox/.style={draw, rounded corners=2pt, fill=#1, minimum width=1.4cm, minimum height=0.5cm, font=\scriptsize, align=center, inner sep=2pt},
        nbox/.default={blue!8},
        arr/.style={-{Stealth[length=4pt]}, thick, #1},
        arr/.default={gray!70},
        blocked/.style={-{Stealth[length=4pt]}, thick, red!50, dashed},
        idarr/.style={-{Stealth[length=4pt]}, thick, blue!60},
        feedarr/.style={-{Stealth[length=4pt]}, thick, orange!60, dashed},
    ]
    % Row 1: Exogenous
    \node[nbox=green!12] (sanc) at (0, 0) {sanctions\\relief};
    \node[nbox=green!12] (noopec) at (5.5, 0) {non-OPEC\\growth};
    \node[nbox=green!12] (demand) at (8.5, 0) {global oil\\demand};
    \node[nbox=green!12] (spare) at (3.2, 0) {OPEC spare\\capacity};

    % Row 2: Venezuelan chain
    \node[nbox] (fdi) at (-0.5, -1.6) {FDI oil\\sector};
    \node[nbox] (vzprod) at (2.2, -1.6) {VZ\\production};
    \node[nbox=red!8] (opec) at (4.8, -1.6) {OPEC quota\\response};

    % Row 3: Global market
    \node[nbox=blue!12] (supply) at (4.8, -3.2) {global oil\\supply};
    \node[nbox=blue!12] (balance) at (6.8, -3.2) {supply-demand\\balance};
    \node[nbox=yellow!15] (brent) at (9.2, -3.2) {\textbf{Brent}\\price};

    % Row 2 right: Heavy sour
    \node[nbox] (heavy) at (9.2, -1.6) {heavy-sour\\discount};

    % Edges: Structural path (black, thick)
    \draw[arr] (sanc) -- node[above, font=\tiny] {+0.15} (vzprod);
    \draw[idarr] (vzprod) -- node[left, font=\tiny] {1:1} (supply);
    \draw[idarr] (supply) -- node[above, font=\tiny] {+1} (balance);
    \draw[arr] (balance) -- node[above, font=\tiny] {-10} (brent);

    % Edges: Identity to supply
    \draw[idarr] (noopec) -- node[right, font=\tiny] {1:1} (supply);
    \draw[idarr] (opec) -- node[right, font=\tiny] {1:1} (supply);
    \draw[idarr] (demand) -- node[right, font=\tiny, pos=0.3] {-1} (balance);

    % Edges: Investment channel
    \draw[arr] (sanc) -- node[left, font=\tiny] {+3.0} (fdi);
    \draw[arr] (fdi) -- node[above, font=\tiny] {+0.05} (vzprod);

    % Edges: OPEC reaction (blocked)
    \draw[blocked] (vzprod) -- node[above, font=\tiny, red!70] {\textit{-0.5}} (opec);
    \draw[arr] (spare) -- (opec);

    % Edges: Heavy sour channel
    \draw[arr=gray!50] (vzprod) -- node[above, font=\tiny] {+5.0} (heavy);

    % Edges: Feedback loop
    \draw[feedarr] (brent) to[bend left=20] node[right, font=\tiny] {+0.05} (fdi);

    % Legend
    \node[font=\tiny, anchor=north west] at (-1.5, -4.2) {
        \textcolor{gray!70}{\rule{0.5cm}{1.5pt}} causal \quad
        \textcolor{blue!60}{\rule{0.5cm}{1.5pt}} identity \quad
        \textcolor{red!50}{- - -} reaction fn (blocked) \quad
        \textcolor{orange!60}{- - -} feedback
    };

    \end{tikzpicture}
    \caption{Venezuela oil supply unlock DAG (11~nodes, 13~edges). The structural path (sanctions $\to$ production $\to$ supply $\to$ balance $\to$ Brent price) consists entirely of \texttt{IDENTIFIED\_CAUSAL} edges. The OPEC reaction function (red dashed) is correctly blocked in all propagation modes. Green nodes are exogenous; the yellow target is Brent crude price.}
    \label{fig:venezuela-dag}
\end{figure}

Unlike the Kazakhstan case study (which uses estimated coefficients from data), the Venezuela DAG uses validated literature estimates as immutable evidence---enabling structural propagation without a data pipeline. Table~\ref{tab:venezuela-propagation} shows the propagation results.

\begin{table}[t]
\centering
\caption{Propagation results for the Venezuela oil supply unlock DAG. The structural path produces a well-identified estimate; the investment channel adds magnitude but with wider confidence intervals.}
\label{tab:venezuela-propagation}
\small
\begin{tabular}{@{}llrrl@{}}
\toprule
\textbf{Scenario} & \textbf{Mode} & \textbf{Effect} & \textbf{95\% CI} & \textbf{Paths} \\
\midrule
Sanctions relief (1 unit) & STRUCTURAL & $-\$1.50$/bbl & $[-2.82, -0.18]$ & 1 open, 3 blocked \\
Sanctions relief (1 unit) & REDUCED\_FORM & $-\$3.00$/bbl & $[-6.40, +0.40]$ & 2 open, 2 blocked \\
$+$0.4 mb/d VZ production & STRUCTURAL & $-\$4.00$/bbl & $[-6.35, -1.65]$ & 1 open \\
\bottomrule
\end{tabular}
\end{table}

Three key guardrail behaviors are demonstrated: (1)~the OPEC reaction function edge (\texttt{reaction\_function} type) is blocked in \emph{all} modes, preventing mechanical propagation of a strategic response that may diverge from historical patterns; (2)~the FDI-investment channel opens only in \texttt{REDUCED\_FORM} mode, correctly reflecting its weaker identification; (3)~the heavy-sour discount channel is gated separately, as Venezuelan 16~API crude is not fungible with light sweet benchmarks. The hedged language guardrails (Section~\ref{sec:mcp}) ensure that Claude uses ``causes'' only for the fully identified structural path and downgrades to ``is associated with'' when reduced-form edges are included.

\subsection{End-to-End Reproducibility}

We run the full agentic pipeline on the KSPI K2 DAG (21~edges) and compare against the expert manual baseline. The pipeline achieves 100\% estimate match (21/21~edges), confirming reproducibility across pipeline modes and adapter dispatch.

\subsection{Issue Detection Effectiveness}

Across 13 pipeline runs on the Kazakhstan case study, the 38-rule engine detects 1{,}479 issues in total, of which 1{,}333 (90.1\%) are \texttt{CRITICAL} severity. PatchBot auto-fixes 812 issues (54.9\%), primarily missing unit specifications (\texttt{UNIT\_MISSING\_IN\_EDGECARD}). Of the 667 remaining open issues, only 19 (1.3\% of total) require human review---demonstrating that the system effectively triages the review burden.
...
\paragraph{Limited case studies.}
Our primary case study is Kazakhstan bank stress testing, with a second demonstration DAG modeling Venezuelan oil supply dynamics (11~nodes, 13~edges, modeling sanctions relief through FDI and production channels to Brent crude prices). While the platform is domain-agnostic, the data clients and variable catalogs are currently specific to these use cases.
...
\oc{} demonstrates that automation and transparency in causal inference are not mutually exclusive. By treating every modeling decision as an auditable event---logged in a hash-chained ledger, gated by identifiability screens and refutation tests, and protected by anti-p-hacking policies---the platform enables researchers to move faster while maintaining the evidentiary standards that credible causal claims demand. The always-on sentinel loop ensures that this rigor is continuous rather than episodic: the DAG is re-validated after every change, schema regressions are auto-healed, and interactive review panels are surfaced the moment estimation completes. With 19 estimation adapters spanning econometric and ML-based methods, data-driven causal discovery (PC, GES, FCI, NOTEARS), adaptive data discovery via LLM tool-use, a 4-agent agentic pipeline, LLM-assisted literature extraction, a principle-based DAG Critic with causal logic probes and estimation-aware auto-repair, a KnowledgeScout agent for LLM-backed formula research, a DAGHealthSnapshot eval framework for regression detection, a data-availability oracle, a Model Context Protocol server for AI-assisted querying, and a companion economics guardrails plugin that enforces hedged language and validates interpretation at the moment AI assistants translate results into prose, \oc{} bridges expert-specified and data-driven structure learning within a unified governance framework. We release \oc{} as open-source software to support reproducible causal inference research.
...
The \oc{} issue registry contains 38 detection rules organized into seven categories. Table~\ref{tab:issue-registry} presents all rules.

\begin{table}[htbp]
\centering
\caption{Full Issue Detection Registry (38 Rules). Trigger: \texttt{pre} = pre-run, \texttt{post} = post-run, \texttt{xrun} = cross-run.}
...
\texttt{SIG\_NOT\_IDENTIFIED} & HIGH & id & post & No & $p<0.05$ but claim $\neq$ IDENTIFIED\_CAUSAL \\
\midrule
...
\texttt{ORPHAN\_NODE\_UNCON} & MED & rpt & pre & Yes & Node not connected to any edge \\
\texttt{UNIT\_MISSING\_CARD} & CRIT & rpt & post & Yes & EdgeCard missing treatment/outcome unit \\
\midrule
\multicolumn{6}{l}{\textit{Structural \& Data Pipeline}} \\
\texttt{TARGET\_UNREACHABLE} & CRIT & dag & pre & No & Target node not reachable from any source \\
\texttt{IDENTITY\_DEPS\_INCOM} & HIGH & dag & pre & Yes & Identity edge missing input dependencies \\
\texttt{SINK\_NODE\_NOT\_TARG} & MED & dag & pre & Yes & Non-target sink node (dead end) \\
\texttt{EDGE\_ID\_SYNTAX\_INV} & HIGH & dag & pre & Yes & Edge ID does not match \texttt{from\_to\_to} pattern \\
\texttt{ID\_FORMULA\_DBL\_XFM} & MED & dag & pre & No & Identity formula applies double log/diff \\
\texttt{LOADER\_FAILED} & HIGH & data & pre & No & Data loader raised exception \\
\texttt{DATA\_INSUFF\_EDGE} & HIGH & data & pre & No & Insufficient observations for estimation \\
\texttt{ALIGNMENT\_FAILURE} & MED & data & pre & No & Frequency alignment produced empty series \\
\bottomrule
\end{tabular}
\end{table}
...
The following annotated session illustrates the \oc{} Query REPL with natural-language query parsing, guardrail enforcement, and hedged narration. Figure~\ref{fig:query-repl} shows the literature edge-proposal workflow.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth, trim=0 0 0 0, clip]{fig_query_repl.png}
    \caption{Query REPL --- Literature Edge Proposal. The REPL searches academic papers via Semantic Scholar, OpenAlex, and arXiv, extracts causal claims using the LLM, and presents proposed edges with confidence scores, edge types, and evidence counts. The analyst reviews each proposal interactively (accept/reject).}
    \label{fig:query-repl}
\end{figure}
