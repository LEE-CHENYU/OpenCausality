...
\begin{figure}[t]
    ...
    \node[component] at (-2.8, 4.5) {YAML Parser};
    \node[component] at (-0.5, 4.5) {Node/Edge Types};
    \node[component] at (2.0, 4.5) {Acyclicity Check};
    \node[component] at (4.5, 4.5) {Unit Validation};

    % Layer 2: Estimation
    \node[layer, fill=green!8] (est) at (0, 3.0) {};
    \node[layerlabel] at (-6.4, 3.0) {Estimation};
    \node[component] at (-3.8, 3.0) {Adapter Registry};
    \node[component] at (-1.5, 3.0) {LP / Panel FE};
    \node[component] at (0.9, 3.0) {IV / DiD / RDD};
    \node[component] at (3.2, 3.0) {Synth Control};
    \node[component] at (5.2, 3.0) {Identity};

    % Layer 3: Inference
    \node[layer, fill=orange!8] (inf) at (0, 1.5) {};
    \node[layerlabel] at (-6.4, 1.5) {Inference};
    \node[component] at (-3.5, 1.5) {3-Stage ID Screen};
    \node[component] at (-1.0, 1.5) {TSGuard (7 diag.)};
    \node[component] at (1.8, 1.5) {29-Rule Issues};
    \node[component] at (4.3, 1.5) {7-Guardrail Propagation};
    ...
    % Feedback arrow from governance back to inference
    \draw[arr, dashed] (6.2, 0.3) -- (6.2, 1.2) -- (5.8, 1.2);

    % Side labels
    \node[font=\tiny, rotate=90, gray] at (6.6, 0.75) {feedback};

    \end{tikzpicture}
    \caption{Architecture of \oc{}. The DAG layer specifies causal structure. The estimation layer dispatches to 11 adapters. The inference layer screens identifiability, detects issues, and propagates effects with 7 guardrails. The governance layer logs all decisions in a hash-chained audit trail with HITL gates. Dashed arrow indicates governance feedback (e.g., issue-triggered re-estimation).}
...
\begin{figure}[t]
    ...
    % Top issues
    \node[font=\tiny, text width=3cm, align=left, gray] at (5.2, -1.5) {Top: SIG\_NOT\_ID (492)\\LOO\_INSTAB (87)\\RATING\_CONFLICT (59)};
    \end{tikzpicture}
