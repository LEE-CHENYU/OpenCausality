...
The past decade of data science was shaped by a single economic force: the plummeting cost of storage and compute made it cheaper to collect everything and predict than to think carefully and explain. Predictive machine learning flourished not because prediction is what organizations need most, but because big data made it the path of least resistance. Correctness---understanding \emph{why} something happens---was never the optimization target.

This matters because prediction, causal interpretation, and causal inference are fundamentally different things. A predictive model can exploit any statistical regularity---including spurious correlations and confounded associations---and still forecast well, without representing the actual data generating process at all. Causal interpretation goes a step further: it takes an observed correlation and attaches a narrative about cause and effect. But a plausible story about \emph{why} two variables move together is not evidence that intervening on one will change the other. Correlations can arise from confounding, reverse causality, or selection---none of which a predictive model or a post-hoc narrative is designed to detect. Causal inference, by contrast, is the discipline of recovering the data generating process itself: specifying which variables cause which, under what assumptions identification holds, and what would happen under intervention. That is a qualitatively harder problem than prediction, and it is the problem that matters when decisions have consequences.
...
\begin{enumerate}[leftmargin=*, itemsep=2pt]
    ...
    \item A \textbf{30-rule issue detection engine} with a PatchPolicy whitelist that explicitly prohibits control shopping, sample trimming, lag searching, and outcome switching---preventing automated p-hacking while permitting safe auto-fixes (Section~\ref{sec:issues}).
...
We evaluate \oc{} on a Kazakhstan bank stress-testing case study involving 32 nodes and 23 causal edges, comparing expert-built DAGs against both LLM-extracted and data-driven discovery DAGs, and verifying pipeline reproducibility across 204 tests (Section~\ref{sec:experiments}).
...
\oc{} ships with 30 issue-detection rules loaded from a YAML registry. Each rule specifies a severity (\texttt{CRITICAL}, \texttt{HIGH}, \texttt{MEDIUM}, \texttt{LOW}), a scope (edge, node, DAG, run, or cross-run), and whether the issue is auto-fixable or requires human review.
...
We compare an expert-built DAG (32~nodes, 23~edges, approximately 40~hours of construction time) against an LLM-extracted DAG from a single descriptive paragraph about Kazakhstan bank stress testing.
...
\begin{figure}[t]
    ...
    % Expert only
    \node[font=\footnotesize, text width=2cm, align=center] at (-2.5, 0) {20 edges\\{\scriptsize (oil shocks, policy\\rate, IV construct,\\balance sheet, ...)}};
    ...
    % Counts
    \node[font=\scriptsize, gray] at (-1.2, -2.2) {32 nodes, 23 edges};
    \node[font=\scriptsize, gray] at (1.2, -2.2) {18 nodes, 12 edges};
    \end{tikzpicture}
    \caption{Comparison of expert-built and NL-extracted DAGs for the Kazakhstan case study. The 3~matched edges achieve 100\% estimate precision. The NL pipeline discovers 9~novel edges, while missing 20~edges encoding upstream shock transmission and banking balance sheet mechanics.}
    \label{fig:nl2dag}
\end{figure}

The NL pipeline achieves 13\% structural recall---unsurprising given that a single paragraph cannot encode upstream shock transmission (oil supply/demand shocks to exchange rate), monetary policy mechanics, or banking balance sheet identities. However, all 3~matched edges produce statistically equivalent estimates, and the pipeline discovers 9~novel edges. Three are particularly notable:
...
We run the full agentic pipeline on the KSPI K2 DAG (23~edges) and compare against the expert manual baseline. The pipeline achieves 100\% estimate match (23/23~edges), confirming reproducibility across pipeline modes and adapter dispatch.

\subsection{Issue Detection Effectiveness}

Across 13 pipeline runs on the Kazakhstan case study, the 30-rule engine detects 1{,}479 issues in total, of which 1{,}333 (90.1\%) are \texttt{CRITICAL} severity. PatchBot auto-fixes 812 issues (54.9\%), primarily missing unit specifications (\texttt{UNIT\_MISSING\_IN\_EDGECARD}). Of the 667 remaining open issues, only 19 (1.3\% of total) require human review---demonstrating that the system effectively triages the review burden.
...
The \oc{} issue registry contains 30 detection rules organized into six categories. Table~\ref{tab:issue-registry} presents all rules.

\begin{table}[htbp]
\centering
\caption{Full Issue Detection Registry (30 Rules). Trigger: \texttt{pre} = pre-run, \texttt{post} = post-run, \texttt{xrun} = cross-run.}
