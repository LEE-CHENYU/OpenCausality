...
class SystemReport:
    ...
    def to_markdown(self) -> str:
        ...
        if self.blocked_edges:
            ...
            lines.append("")
            lines.append(
                "> **Remediation:** Blocked edges need either additional data, "
                "a different identification strategy, or manual resolution via the "
                "HITL panel. Run `opencausality monitor` to track progress."
            )
            lines.append("")
        ...
        if self.critical_path:
            ...
            lines.append("")
            if not self.critical_path.path_complete:
                lines.append(
                    "> **Note:** The critical path is incomplete. Counterfactual "
                    "predictions through this path will be unreliable until all "
                    "blocking edges are resolved."
                )
                lines.append("")
        ...
        if weak_edges:
            lines.append("## Weak Edges (Credibility < 0.60)")
            lines.append("")
            for s in weak_edges:
                lines.append(f"- **{s.edge_id}:** {s.credibility_score:.2f}")
            lines.append("")
            lines.append(
                "> **What makes an edge weak:** Low credibility scores typically "
                "result from failed diagnostics, small sample sizes, fragile "
                "inference (LOO instability), or missing identification strategies. "
                "Consider re-estimating with a stronger design, increasing the sample, "
                "or downgrading the edge to diagnostic-only if it cannot support "
                "causal claims."
            )
            lines.append("")

        lines.append("---")
        lines.append(f"*Generated by OpenCausality Agentic DAG System*")
