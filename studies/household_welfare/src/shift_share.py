...
Implements panel DiD with exposure x shock interactions.

Study Design (v4):
- MAIN SPEC: Oil exposure only (E_oil_r x oil shocks)
  - Clean identification: regional oil exposure x exogenous oil shock
  - Time FE absorbs national cyclical effects

- ROBUSTNESS SPEC: Add cyclical proxy to check beta stability
...
# y_{r,t} = alpha_r + delta_t + beta(E_oil_r x Shock_oil_t) + u_{r,t}
...
MAIN_SPEC = ShiftShareSpec(
    ...
    entity_effects=True,  # alpha_r: region fixed effects
    time_effects=True,  # delta_t: absorbs national cycle
...
# =============================================================================
# Tests if beta is stable when adding noisy cyclical exposure control:
# y_{r,t} = alpha_r + delta_t + beta(E_oil_r x Shock_oil_t) + theta(E_cyc_proxy_r x Shock_cyc_t) + u_{r,t}
...
class ShiftShareModel:
    """Shift-share regression model with exposure x shock interactions."""
    ...
    def fit_robustness(self) -> RegressionResult:
        ...
        Tests if beta (oil coefficient) is stable when adding noisy cyclical control.
    ...
    def check_beta_stability(self) -> dict[str, Any]:
        """
        Compare beta (oil coefficient) across main and robustness specs.
        ...
        return {
            ...
            "interpretation": (
                "beta is STABLE: adding cyclical proxy does not materially change oil coefficient"
                if abs(pct_change) < 20
                else "beta is NOT STABLE: adding cyclical proxy changes oil coefficient significantly"
            ),
        }
    ...
    def summary(self, spec_name: str | None = None) -> str:
        ...
        for name, res in results.items():
            ...
            lines.append(f"R-squared within: {res.r2_within:.4f}")
