"""
Panel data construction for household welfare study.
...
from shared.data.kazakhstan_bns import BNSDataType
...
class PanelBuilder:
    ...
    def _add_income(
        ...
        if income_df.empty:
            raise ValueError(
                ...
                "Run 'kzresearch welfare fetch-data bns' to download data, or check BNS API status."
            )
    ...
    def _compute_debt_exposure(
        ...
        if expenditure_df.empty:
            logger.warning("No expenditure data available. E_debt_r will be missing.")
            return panel
        ...
        if "debt_share" not in expenditure_df.columns:
            logger.warning(f"Expenditure data missing 'debt_share' column.")
            return panel

        if "year" not in expenditure_df.columns:
            logger.warning("Expenditure data missing 'year' column.")
            return panel
        ...
        if baseline.empty:
            logger.warning(
                f"No expenditure data in baseline period ({baseline_start}-{baseline_end})."
            )
            return panel
    ...
    def _add_cyclical_proxy(self, panel: pd.DataFrame) -> pd.DataFrame:
        ...
        that the oil exposure coefficient (beta) is stable when controlling
    ...
    def _create_interactions(self, panel: pd.DataFrame) -> pd.DataFrame:
        """
        Create exposure x shock interaction terms.

        Main spec (v4): Uses E_oil_r x oil shocks only
        Robustness: Adds E_cyc_proxy_r x global activity
        """
        # === MAIN SPECIFICATION: Oil exposure interactions ===

        # Oil exposure x oil supply shock (PRIMARY)
        ...
        # Oil exposure x oil demand shock
        ...
        # True cyclical exposure x global activity (if BNS data available)
        ...
        # Cyclical PROXY x global activity (for robustness checks)
